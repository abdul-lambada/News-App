...
@section('content')
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div id="default-carousel" class="relative w-full mb-8 z-0" data-carousel="slide" data-carousel-interval="6000">
                ...
                @foreach ($news->take(5) as $index => $newsItem)
                    <div class="{{ $index === 0 ? '' : 'hidden' }} duration-700 ease-in-out" data-carousel-item>
                        <img src="{{ asset('storage/' . $newsItem->image_url) }}"
                            class="absolute block w-full h-full object-cover" alt="{{ $newsItem['title'] }}">
            ...
            <div class="absolute z-30 flex -translate-x-1/2 bottom-4 left-1/2 space-x-3 rtl:space-x-reverse">
                @foreach ($news->take(5) as $index => $newsItem)
                    <button type="button" class="w-2.5 h-2.5 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-gray-300' }}"
        ...
        <div id="news-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($paginatedNews as $newsItem)
                <a href="{{ route('news.detail', $newsItem['id']) }}"
                    ...
                    <img class="object-cover w-full h-40 rounded-t-lg" src="{{ asset('storage/' . $newsItem->image_url) }}"
                        alt="{{ $newsItem['title'] }}">
    ...
    <!-- Start of tabs -->
    <div class="ps-4 mb-4 border-b border-gray-200 dark:border-gray-700">
        <ul class="ps-4 flex flex-wrap mb-px text-sm font-medium text-center" id="default-tab"
            data-tabs-toggle="#default-tab-content" role="tablist">
            <li class="me-2" role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg" id="editor-choice-tab"
                    data-tabs-target="#editor-choice" type="button" role="tab" aria-controls="editor-choice"
                    aria-selected="true">Feature</button>
            </li>
            <li class="me-2" role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg" id="komunitas-tab" data-tabs-target="#komunitas"
                    type="button" role="tab" aria-controls="komunitas" aria-selected="false">Komunitas</button>
            </li>
            <li class="me-2" role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg" id="opini-tab" data-tabs-target="#opini"
                    type="button" role="tab" aria-controls="opini" aria-selected="false">Opini</button>
            </li>
        </ul>
    </div>
    <div id="ps-4 default-tab-content">
        <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="editor-choice" role="tabpanel"
            aria-labelledby="editor-choice-tab">
            @if ($editorChoiceMain)
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        {{ \Carbon\Carbon::parse($editorChoiceMain->date)->format('F j, Y') }}</p>
                </div>
            @else
                <div class="mb-4">
            ...
            @endif
            <div class="ps-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="komunitas" role="tabpanel"
            aria-labelledby="komunitas-tab">
            @if ($komunitasMain)
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        {{ \Carbon\Carbon::parse($komunitasMain->date)->format('F j, Y') }}</p>
                </div>
            @else
                <div class="mb-4">
        ...
        <div class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="opini" role="tabpanel"
            aria-labelledby="opini-tab">
            @if ($opiniMain)
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        {{ \Carbon\Carbon::parse($opiniMain->date)->format('F j, Y') }}</p>
                </div>
            @else
                <div class="mb-4">
