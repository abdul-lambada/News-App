<nav class="bg-gray-100 rounded-lg border-gray-200 dark:bg-gray-900">
    <div class="container mx-auto flex flex-wrap items-center justify-between p-4 space-x-4">
        <a href="{{ url('/') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <span class="self-center text-lg md:text-2xl font-semibold whitespace-nowrap dark:text-white">News
                Talenthub</span>
        </a>
        <div class="flex items-center space-x-4 md:order-2">
            <button type="button" aria-controls="search-bar" aria-expanded="false"
                class="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 md:hidden"
                id="search-toggle">
            ...
            <button type="button"
                ...
                id="menu-toggle" aria-controls="navbar-menu" aria-expanded="false">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-menu">
            <ul
                class="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-gray-100 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                <li>
                    <a href="{{ url('/') }}"
                ...
                @foreach ($categories->take(5) as $category)
                    <li>
                        <a href="{{ route('category', ['category' => strtolower($category->name)]) }}"
                            class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">{{ $category->name }}</a>
                    </li>
                @endforeach
                @if ($categories->count() > 5)
                    <li class="relative">
                        <button
                            class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent flex items-center"
                        ...
                        <div class="absolute z-10 hidden w-48 py-2 mt-2 bg-gray-100 border rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700"
                            id="more-categories">
                ...
                <li class="relative">
                    ...
                    <div class="absolute z-10 hidden w-48 py-2 mt-2 bg-gray-100 border rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700"
                        ...
                        @foreach ($regions as $region)
                            <a href="{{ route('region', ['region' => strtolower($region->name)]) }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700">{{ $region->name }}</a>
                        @endforeach
                    </div>
                </li>
        </div>
    ...
    <div class="fixed inset-x-0 top-16 z-50 hidden bg-white dark:bg-gray-800 md:hidden" id="mobile-menu">
        <ul class="flex flex-col font-medium p-4">
            <li>
                <a href="{{ url('/') }}" class="block py-2 px-3 text-white bg-blue-700 rounded dark:bg-blue-600"
                    aria-current="page">Home</a>
            </li>
            @foreach ($categories->take(5) as $category)
                <li>
                    <a href="{{ route('category', ['category' => strtolower($category->name)]) }}"
                        class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">{{ $category->name }}</a>
                </li>
            @endforeach
            @if ($categories->count() > 5)
                <li class="relative">
                    <button
                        class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 flex items-center"
